...
struct common_speculative_impl {
    ...
    virtual bool supports_process_prefixes() const { return false; }

    virtual bool process_prefixes(const llama_batch & batch, const std::vector<int32_t> & /*rows_by_seq*/) {
...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    bool process_impl(const llama_batch & batch_in, const std::vector<int32_t> * rows_by_seq) {
        if (batch_in.n_tokens <= 0) {
            return true;
        }
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            if (rows_by_seq != nullptr) {
                GGML_ASSERT((size_t) seq_id < rows_by_seq->size());
                const int32_t n_prefix = rows_by_seq->at(seq_id);
                if (n_prefix == 0) {
                    continue;
                }
                if (n_prefix > 0) {
                    n_rows = std::min(n_rows, n_prefix);
                }
            }
    ...
    bool supports_process_prefixes() const override {
        return true;
    }

    bool process_prefixes(const llama_batch & batch_in, const std::vector<int32_t> & rows_by_seq) override {
        return process_impl(batch_in, &rows_by_seq);
    }
...
bool common_speculative_can_process_prefixes(common_speculative * spec) {
    ...
    for (auto & impl : spec->impls) {
        if (!impl->supports_process_prefixes()) {
            return false;
        }
    }
...
bool common_speculative_process_prefixes(common_speculative * spec, const llama_batch & batch, const std::vector<int32_t> & rows_by_seq) {
    ...
    for (auto & impl : spec->impls) {
        result = result && impl->process_prefixes(batch, rows_by_seq);
    }
